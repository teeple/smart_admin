...
function getCurrentTime() {
  ...
  var now_formatted = hours + ":" + minutes + ":" + seconds + ampm;
...
$(document).ready(function() {
  ...
  var socket = io.connect("222.235.208.10:3000");
  var myRoomID = null;
  ...
  $("#chat-body").bind("DOMSubtreeModified",function() {
    $("#chat-body").animate({
...
socket.on("history", function(data) {
  ...
  socket.on("update-people", function(data){
    ...
    $.each(data.people, function(a, obj) {
    $('#chat-users').append('<li><a href="javascript:void(0);"><img src="img/avatars/sunny.png" alt=""> '
                          + obj.name 
                          + '<span class="state"><i class="fa fa-circle txt-color-green pull-right"></i></span></a></li>');
      //peopleOnline.push(obj.name);
    });
  ...
  socket.on("chat", function(person, msg) {
    now_formatted = getCurrentTime();

    // $("#msgs").append("<li class='message'><strong><span class='text-success'>" + person.name + "</span></strong>: " + msg + "</li>");
    $("#msgs").append('<li class="message"><img src="img/avatars/sunny.png" class="online" alt=""><div class="message-text"><time> '
                + now_formatted 
                + '</time> <a href="javascript:void(0);" class="username">'
                + person.name 
                + '</a>'
                + msg + '</div></li>');
